...
- figma 시안에 따라 구현했습니다. 모델별 어느정도의 차이는 나지 않지만, 1대1로 비교하면 조금씩 크기가 다른것이 있을것입니다.
...
- 모두 웹 UI에서 구현하였으며, 잘 돌아갑니다. 제가 아는 선에서는요. 기타 주목할 구현 점은 다음과 같습니다.

- 토스트 팝업: AI의 힘을 빌어서 만들었습니다. 딱 버튼을 클릭했을때, 분명한 상호작용을 했다는 이펙트가 있으면 좋겠다고 생각했는데, 이것이 딱인것 같았습니다.
- 렌더링 최적화: 1200000개 정도의 로또를 구입해도, 버벅이진 않습니다. 그 이상은 구입 과정이 조금 오래 걸릴뿐이지, 이론대로라면 몇 조장을 사도 렌더링 자체는 별 탈 없이 작동할겁니다. Intersection Observer를 사용했습니다. 아직까지 코드가 완벽히 이해가진 않습니다.
- 로또 번호 복사 기능: 카멜(크루) 것이 좋아 보여서 한두시간 들여서 뚝딱 만들었습니다. 그리고 그것을 붙여 넣기 할수도 있어요. 뭐에 쓸지는 모르겠지만..
